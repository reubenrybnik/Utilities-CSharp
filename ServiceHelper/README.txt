...
Other useful/interesting pieces of code include:
 ...
 - A set of methods that can be called at or near safe stopping points to allow the service to be gracefully stopped in the middle of a loop
 - A "reusable" thread implementation that allows for thread aborts if necessary
 - Some cleverness around exception handling/not handling to help improve the context of debugging "unhandled" exceptions (the program will stop execution at the exception instead of jumping to a catch block without needing to enable breaking on thrown exceptions)
